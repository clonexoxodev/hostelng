...
import {
  ...
  Clock, PhoneCall, ChevronDown, Home, MapPin,
} from 'lucide-react';
...
const AgentInquiries = () => {
  ...
  const [activeTab, setActiveTab] = useState<'inquiries' | 'home_requests'>('inquiries');
  ...
  const checkAuth = async () => {
    ...
    loadHomeRequests(session.user.id);
  };
  ...
  const loadHomeRequests = async (userId: string) => {
    try {
      const { data, error } = await supabase
        .from('home_requests')
        .select('*')
      ...
      console.error('Failed to load home requests');
    }
  };
  ...
  const HOME_REQUEST_STATUSES = [
    { value: 'assigned',    label: 'Assigned',    color: 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400' },
    { value: 'contacted',   label: 'Contacted',   color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-950/40 dark:text-yellow-400' },
    { value: 'in_progress', label: 'In Progress', color: 'bg-orange-100 text-orange-700 dark:bg-orange-950/40 dark:text-orange-400' },
    { value: 'completed',   label: 'Completed',   color: 'bg-green-100 text-green-700 dark:bg-green-950/40 dark:text-green-400' },
    { value: 'closed',      label: 'Closed',      color: 'bg-secondary text-muted-foreground' },
  ];

  const updateHomeRequestStatus = async (id: string, status: string) => {
    setUpdatingId(id);
    try {
      const { error } = await supabase
        .from('home_requests')
  ...
  const newHomeRequests = homeRequests.filter((r) => r.status === 'assigned').length;
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                {newHomeRequests > 0 && (
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-accent text-accent-foreground">
                    {newHomeRequests} home request{newHomeRequests !== 1 ? 's' : ''}
                  </span>
                )}
              </div>
              <p className="text-muted-foreground text-sm">
                {inquiries.length} listing inquiries · {homeRequests.length} home requests
              </p>
            </div>
          ...
          <div className="flex gap-2 mb-6 border-b border-border">
            ...
            >
              Listing Inquiries
              {inquiries.length > 0 && (
                <span className="ml-2 text-xs bg-secondary text-muted-foreground px-1.5 py-0.5 rounded-full">
                  {inquiries.length}
                </span>
              )}
            </button>
            <button
              onClick={() => { setActiveTab('home_requests'); setFilterStatus('all'); }}
              className={`px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors -mb-px flex items-center gap-1.5 ${
                activeTab === 'home_requests'
                  ? 'border-primary text-primary'
                  : 'border-transparent text-muted-foreground hover:text-foreground'
              }`}
            >
              ...
              {homeRequests.length > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                  newHomeRequests > 0 ? 'bg-accent text-accent-foreground' : 'bg-secondary text-muted-foreground'
                }`}>
                  {homeRequests.length}
                </span>
              )}
                  ...
                  {filtered.map((inq) => (
                    ...
                    >
                        ...
                        <div className="flex items-start justify-between gap-3 mb-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                              <span className="text-primary font-bold">{inq.student_name?.charAt(0)?.toUpperCase()}</span>
                            </div>
                            <div>
                      ...
                      <div className="grid grid-cols-3 border-t border-border divide-x divide-border">
                        ...
                        <a href={buildWhatsAppUrl(inq.student_phone, `Hi ${inq.student_name}, I'm responding to your inquiry about ${inq.hostel_title} on HostelNG.`)}
                          ...
                          <svg className="w-4 h-4 fill-[#25D366] group-hover:fill-white transition-colors" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                          WhatsApp
                        </a>
                        <a href={`mailto:${inq.student_email}?subject=${encodeURIComponent(`Re: Your inquiry about ${inq.hostel_title}`)}&body=${encodeURIComponent(`Hi ${inq.student_name},\n\nThank you for your inquiry about ${inq.hostel_title} on HostelNG.\n\n`)}`}
            ...
            <>
              {homeRequests.length === 0 ? (
                <div className="bg-card rounded-2xl border border-border p-12 text-center">
                  ...
                  <h3 className="font-display text-lg font-bold text-foreground mb-2">No home requests yet</h3>
                  <p className="text-muted-foreground text-sm">
                    When the Super Admin assigns housing requests to you, they will appear here.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {homeRequests.map((req) => {
                    const statusMeta = HOME_REQUEST_STATUSES.find((s) => s.value === req.status);
                    return (
                      <div
                        key={req.id}
                        className={`bg-card rounded-2xl border overflow-hidden transition-shadow hover:shadow-card-hover ${
                          req.status === 'assigned' ? 'border-accent/50' : 'border-border'
                        }`}
                      >
                        <div className="p-5">
                          {/* Badge + header */}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  <p className="font-semibold text-foreground leading-tight">{req.student_name}</p>
                                ...
                                <div className="flex items-center gap-3 mt-0.5 flex-wrap text-xs text-muted-foreground">
                                  ...
                                  <span>{HOUSING_LABELS[req.housing_type] || req.housing_type}</span>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <div className="relative">
                                <select
                                  ...
                                  onChange={(e) => updateHomeRequestStatus(req.id, e.target.value)}
                                  className="appearance-none text-xs px-3 py-2 pr-7 rounded-lg border border-border bg-background text-foreground cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50"
                                >
                                  {HOME_REQUEST_STATUSES.map((s) => (
                                    <option key={s.value} value={s.value}>{s.label}</option>
                                  ))}
                                </select>
                                <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground pointer-events-none" />
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm mb-4">
                            ...
                            {req.move_in_date && (
                              <div className="flex items-center gap-2 text-muted-foreground">
                                <Calendar className="w-3.5 h-3.5 shrink-0 text-primary/60" />
                                <span>Move-in: {new Date(req.move_in_date).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                              </div>
                            )}
                          </div>
                        ...
                        {/* Contact bar */}
                        <div className="grid grid-cols-3 border-t border-border divide-x divide-border">
                          <a href={`tel:${req.student_phone}`}
                            className="flex flex-col items-center justify-center gap-1 py-3 text-xs font-semibold text-foreground hover:bg-primary hover:text-primary-foreground transition-colors group">
                            <PhoneCall className="w-4 h-4 text-primary group-hover:text-primary-foreground transition-colors" />
                            Call Student
                          </a>
                          <a href={buildWhatsAppUrl(req.student_phone, `Hi ${req.student_name}, I'm a property owner on HostelNG. I'm reaching out about your housing request for ${req.preferred_location}.`)}
                            target="_blank" rel="noopener noreferrer"
                            className="flex flex-col items-center justify-center gap-1 py-3 text-xs font-semibold text-foreground hover:bg-[#25D366] hover:text-white transition-colors group">
                            <svg className="w-4 h-4 fill-[#25D366] group-hover:fill-white transition-colors" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            WhatsApp
                          </a>
                          <a href={`mailto:${req.student_email}?subject=${encodeURIComponent('Re: Your Housing Request on HostelNG')}&body=${encodeURIComponent(`Hi ${req.student_name},\n\nI'm a property owner on HostelNG reaching out about your housing request for ${req.preferred_location}.\n\n`)}`}
                            className="flex flex-col items-center justify-center gap-1 py-3 text-xs font-semibold text-foreground hover:bg-secondary transition-colors group">
                            <Mail className="w-4 h-4 text-primary/70 group-hover:text-primary transition-colors" />
                            Email Student
                          </a>
